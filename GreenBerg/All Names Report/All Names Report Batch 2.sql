--declare @date_create AS date = '2020-01-01';
--declare @date_create_end AS date = '2021-02-01';
--declare @providerRole AS varchar(50) = 'Medical Practice'; 
...
WHERE
	 (SELECT TOP 1 provider_role_list.role FROM provider,provider_roles,provider_role_list WHERE names.id=provider.id AND provider.id=provider_roles.providerid 
		AND  provider_roles.providerrolelistid=provider_role_list.id) in  (
																			'DOCTOR',
																			'EXPERT WITNESS',
																			'GB DR',
																			'HEALTH INS',
