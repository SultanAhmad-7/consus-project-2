...
FROM 
	names
WHERE
	 ((SELECT TOP 1 provider_role_list.role FROM provider,provider_roles,provider_role_list 
		WHERE names.id=provider.id AND provider.id=provider_roles.providerid AND  provider_roles.providerrolelistid=provider_role_list.id) is null OR
(SELECT TOP 1 provider_role_list.role FROM provider,provider_roles,provider_role_list WHERE names.id=provider.id AND provider.id=provider_roles.providerid 
		AND  provider_roles.providerrolelistid=provider_role_list.id) = '')		
