...
DECLARE @rptName varchar(100) = 'Cs Pending Pi Intakes (Summary)';
DECLARE @rptDesc varchar(500) = 'Cs Pending Pi Intakes (Summary)';
...
SELECT 
	COUNT(intake_taken) AS Total_Count
	,ROUND(AVG(TRY_CAST(Days_Since_Last_Review AS float)),2) AS AVG_Days_Last_Review
	,ROUND(AVG(IIF(TRY_CAST(Days_Since_Last_Review AS float) > 0,TRY_CAST(Days_Since_Last_Review AS float),0)),2) AS AVG_Days_Last_Review2
	,SUM(IIF(Days_Since_Last_Review > 3,1,0)) AS Number_Of_Days
FROM
	cteAllData AS cte;',@baseTableId)

-- insert the columns, one row per column in the select
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'Total_Count', @newReport, N'Total Pending', 0)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'Avg_Days_Last_Review', @newReport, N'Average Number Of Days', 1)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'Avg_Days_Last_Review2', @newReport, N'Average Position Number Of Days', 2)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'Number_Of_Days', @newReport, N'Total Positive Cases With Number Of Days >3', 3)
