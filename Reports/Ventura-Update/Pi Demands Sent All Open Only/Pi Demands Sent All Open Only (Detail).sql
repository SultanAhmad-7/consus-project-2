...
DECLARE @rptName varchar(100) = 'Pi Demands Sent All Open Only (Detail)';
DECLARE @rptDesc varchar(500) = 'Pi Demands Sent All Open Only (Detail)';
DECLARE @rptId varchar(50) = 'Pi Demands Sent All Open Only';
...
AS (
 ...
 WHERE 
	...
	AND ((@enter_begin_date IS NULL) or (SELECT casedate FROM case_dates, matter_dates WHERE cases.id = case_dates.casesid AND cases.matterid = matter_dates.matterid 
		AND case_dates.datelabelid=matter_dates.datelabelid AND display_order=2) >= @enter_begin_date)
	AND ((@enter_end_date IS NULL) or (SELECT casedate FROM case_dates, matter_dates WHERE cases.id = case_dates.casesid AND cases.matterid = matter_dates.matterid 
	...
	AND matter.matcode NOT IN (''wc'',''PRO'')
 )
 ...
 FROM
	cteAllData as cte
 ORDER BY
	staff_1, case_date_2;',@baseTableId)
