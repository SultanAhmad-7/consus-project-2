...
DECLARE @rptName varchar(100) = 'Pi Atty Case List W Case Status (Group by STF_Person)';
DECLARE @rptDesc varchar(500) = 'Pi Atty Case List W Case Status (Group by STF_Person)';
...
WHERE 
 ...
 SELECT 
	 stf_person
	,count(stf_person) as total_stf_person
 FROM
	cteAllData as cte
 GROUP BY
	stf_person;',@baseTableId)
...
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'total_stf_person', @newReport, N'Total Cases', 1)
