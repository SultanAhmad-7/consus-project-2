...
DECLARE @rptName varchar(100) = 'Pi Atty Case List W Case Status (Group by Lit_Status)';
DECLARE @rptDesc varchar(500) = 'Pi Atty Case List W Case Status (Group by Lit_Status)';
...
WHERE 
 ...
 SELECT 
	stf_person
	,lit_status
	,count(casenum) AS total_lit_status
 FROM 
	cteAllData AS cte
 GROUP BY 
	lit_status, stf_person
 ORDER BY 
	stf_person,lit_status desc;',@baseTableId)

-- insert the columns, one row per column in the select
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'stf_person', @newReport, N'Paralegal', 0)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'lit_status', @newReport, N'Lit Status', 1)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'total_lit_status', @newReport, N'Total Lit Status', 2)
