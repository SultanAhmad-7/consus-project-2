...
DECLARE @rptName varchar(100) = 'Thirty Day Note Topic (Detail)';
DECLARE @rptDesc varchar(500) = 'Thirty Day Note Topic (Detail)';
...
SELECT
	 ...
	 ,staff_3
	 ,party_name 
	,(CASE WHEN (Client_Contact IS NOT NULL  OR  Client_Contact <> '''') THEN 1 ELSE 0 END) AS Client_Contact
FROM 
	cteAllData
ORDER BY
	staff_3;',@baseTableId)
...
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'Client_Contact', @newReport, N'Client Contacted', 3)
