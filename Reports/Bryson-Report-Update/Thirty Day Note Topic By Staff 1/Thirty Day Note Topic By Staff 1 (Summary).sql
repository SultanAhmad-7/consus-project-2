...
DECLARE @rptName varchar(100) = 'Thirty Day Note Topic By Staff 1 (Summary)';
DECLARE @rptDesc varchar(500) = 'Thirty Day Note Topic By Staff 1 (Summary)';
...
SELECT
	...
	,((COUNT(casenum)) - (SUM((CASE WHEN (Client_Contacted IS NOT NULL  OR  Client_Contacted <> '''') THEN 1 ELSE 0 END)))) AS TOTAL_CONTACT_NOT_IN
FROM
	cteAllData;',@baseTableId)

-- insert the columns, one row per column in the select
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'TOTAL_CASES', @newReport, N'Total Open Cases', 0)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'TOTAL_CONTACT__IN', @newReport, N'Total Clients Contacted', 1)
INSERT report_columns_orm (id, binding, reportid, name, col_order) VALUES (newid(), N'TOTAL_CONTACT_NOT_IN', @newReport, N'Total Not Contacted', 2)
